...
if [[ "$ACTION" == "delete" ]]; then
    ...
    execute_or_dryrun "sed -i '/$PROJECTNAME.charlesnchr.com/,+1d' $CONFIG_FILE"
    ...
    # Delete DNS record
    DNS_RECORD_ID=$(curl -X GET "https://api.cloudflare.com/client/v4/zones/$CLOUDFLARE_ZONE_ID/dns_records?type=CNAME&name=$PROJECTNAME.charlesnchr.com" -H "X-Auth-Email: $EMAIL" -H "X-Auth-Key: $CLOUDFLARE_API_KEY" -H "Content-Type: application/json" | jq -r '.result[0].id')
    ...
    if grep -q "$PROJECTNAME.charlesnchr.com" $CONFIG_FILE; then
        echo "Routing/networking/tunnelling has been configured."
        PROJECTPORT=$(awk -v project="$PROJECTNAME.charlesnchr.com" '$0 ~ project {getline; split($0, arr, ":"); print arr[4]}' $CONFIG_FILE)
    else
        ...
        execute_or_dryrun "sed -i '/# Rules map traffic/a - hostname: $PROJECTNAME.charlesnchr.com\n  service: http://localhost:$PROJECTPORT' $CONFIG_FILE"

        # DNS routing with cloudflared
        execute_or_dryrun "cloudflared tunnel route dns '$PROJECTNAME' '$PROJECTNAME.charlesnchr.com'"
    ...
    # Docker build
    execute_or_dryrun "docker build -t '$PROJECTNAME' ."
