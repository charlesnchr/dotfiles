...
{
    gui_search_title = SSH
    gui_search("cmd /C ""%USERPROFILE%\Dropbox\""3.Diverse""\IT\AutoHotKey\Miscellaneous\runs.bat REPLACEME""")
...
else if Pedersen = mail ; Open google inbox
{
    gui_destroy()
...
else if Pedersen = occ ; 
{
    gui_destroy()
    cmd := "cmd /C explorer %userprofile%\CCsvn"
    run %cmd%    
}
else if Pedersen = oth ; 
{
    gui_destroy()
    cmd := "cmd /C explorer %userprofile%\thesis"
    run %cmd%    
}
else if Pedersen = ccc ; 
{
    gui_destroy()
    cmd := "C:\Windows\System32\cmd.exe /C C:\Program^ Files\TortoiseSVN\bin\TortoiseProc.exe /command:commit /path:%userprofile%\CCsvn"
    run %cmd%
}
else if Pedersen = cth ; 
{
    gui_destroy()
    cmd := "C:\Windows\System32\cmd.exe /C C:\Program^ Files\TortoiseSVN\bin\TortoiseProc.exe /command:commit /path:%userprofile%\thesis"
    run %cmd%
}
else if Pedersen = cfd ; 
{
    gui_destroy()
    cmd := "C:\Windows\System32\cmd.exe /C C:\Program^ Files\TortoiseSVN\bin\TortoiseProc.exe /command:commit /path:%userprofile%\firedetect"
    run %cmd%
}
else if Pedersen = ucc ; 
{
    gui_destroy()
    cmd := "C:\Windows\System32\cmd.exe /C C:\Program^ Files\TortoiseSVN\bin\TortoiseProc.exe /command:update /path:%userprofile%\CCsvn"
    run %cmd%
}
else if Pedersen = uth ; 
{
    gui_destroy()
    cmd := "C:\Windows\System32\cmd.exe /C C:\Program^ Files\TortoiseSVN\bin\TortoiseProc.exe /command:update /path:%userprofile%\thesis"
    run %cmd%
}
else if Pedersen = jl ;
{
    gui_destroy()
    cmd := "C:\Users\"
    cmd2 := "\Dropbox\3.Diverse\IT\AutoHotKey\Miscellaneous\JL_cmd.lnk /C %USERPROFILE%\Dropbox\3.Diverse\IT\AutoHotKey\Miscellaneous\jupyterlab.bat"
...
else if Pedersen = cmd ; Ping Google
{
    gui_destroy()
    Run, cmd
}
